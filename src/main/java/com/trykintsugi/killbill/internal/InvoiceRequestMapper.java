...
public final class InvoiceRequestMapper {
    ...
    public static ObjectNode toEstimateRequest(
        ...
        if (invoice.getInvoiceNumber() != null) {
            ...
            // Emit invoice_number as a JSON string (not a number).
    ...
    /**
     * Return adjustment types sent as a return estimate. {@code CREDIT_ADJ} is excluded
     * because it is not a return of a taxable line.
     */
     ...
     * Document id is {@code {invoiceId}:adj-return} to distinguish returns from sales.
     *
    ...
    public static ObjectNode toReturnEstimateRequest(
        ...
        for (final InvoiceItem adj : untaxedAdjustments) {
            ...
            // Lenient: missing linked original → skip rather than fail the invoice.
        ...
        // Always attach document_kind; emit invoice_number as a JSON string.
        final ObjectNode document = (ObjectNode) root.path("documents").get(0);
     ...
     * Plan-less {@code EXTERNAL_CHARGE} uses the shared sentinel product id.
     */
